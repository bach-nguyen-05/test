...
class Tracking(object):
    ...
    def log(self, data, step, backend=None):
        ...
        if "table_content" in data:
            from wandb import Table
            for row in data["table_content"]:
                self.table_content.append([step, row[0], row[1]])
            
            table = Table(columns=["Step", "Name", "Content"])
            for row in self.table_content:
                table.add_data(*row)
            self.logger['wandb'].log({"examples": table, "step": step}, commit=False)
