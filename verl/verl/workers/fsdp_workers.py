...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if rollout_name == 'hf':
        ...
        elif rollout_name == 'vllmConv':
            from verl.workers.rollout.vllm_rollout import vLLMConversationRollout, vllm_mode
            ...
            if vllm_mode == 'customized':
                ...
                rollout = vLLMConversationRollout(model_path=local_path,
                                      config=self.config.rollout,
                                      tokenizer=self.tokenizer,
                                      model_hf_config=self.actor_model_config,
                                      device_mesh=rollout_device_mesh,
                                      external_model_path=self.config.rollout.external_model_path,
                                      external_model_port=self.config.rollout.external_model_port,
                                      max_rounds=self.config.rollout.max_rounds,
                            )
            else:
                raise NotImplementedError("vllm_mode must be 'customized' or 'spmd'")
            log_gpu_memory_usage(f'After building {rollout_name} rollout', logger=None)
